...
# Sensitivity data for R0
R0.HPD <- read_rds("results/R0_HPD_sens.rds") %>%
  ungroup() %>%
  pathosystem_labeller() %>%
    mutate(focal_var = factor(focal_var,
                           levels = c("muV", "sigmaV", "sigmaV_f", "rhoL", "deltaL", "etaV", "betaV"),
                           labels = c(
                             TeX("Adult lifespan"),
...
# Sensitivity data for Topt
Topt.HPD <- read_rds("results/Topt_HPD_sens.rds") %>%
  ungroup() %>%
  pathosystem_labeller() %>%
    mutate(focal_var = factor(focal_var,
                           levels = c("lf", "sigmaV", "sigmaV_f", "rhoL", "deltaL", "etaV", "betaV"),
                           labels = c(
                             TeX("Adult lifespan"),
                             TeX("Biting frequency"),
                             TeX("Eggs per female per day"),
...
# Sensitivity data for critical thermal extrema
...
# Plot R0 vs. temperature curves. One plot for each pathosysten
# One curve for Ross-Macdonald and a few illustrative examples for Chitnis
...
# R0 as a function of temperature, separate curves across values of KH and sigmaH
...
Toptalt_plots <- Toptalt_df %>%
  ...
  # take median of median to hide variation from artifacts of uncertainty
  geom_contour_filled(
...
R0_plot <- R0_df %>%
  ...
  facet_grid(
    ...
    labeller = labeller(.rows = mylabels,
...
Topt_df <- data.Topt %>%
  filter(KH < 10^4 + 1) %>%
  pivot_wider(id_cols = c("system_label", "Model", "sigmaH", "KH"),
              names_from = variable,
...
```{r fig-S3_Topt_plots_varysigmaH, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:Topt_plots} Transmission thermal optimum as a function of vertebrate host availability.'}

# x-axis = vertebrate host biting tolerance
# y-axis = thermal optimum
# color = vertebrate host population density

infinite_skew <- 4500 # 10000
...
CTmin_df <- data.CT %>%
  ...
  mutate(median_proxy = ifelse(is.finite(median), median, 100))

CTmin_df <- inf_skew_func(CTmin_df, infinite_skew, infinite_divide, 8)

maxCTmin <- ceiling(max(filter(CTmin_df, is.finite(median))$median))
...
CTmin_heat_plots <- CTmin_df %>%
  ...
  # y = vertebrate host population density,
  # color = width of parasite thermal median_CTwidth
  ggplot(aes(x = KH, y = sigmaH_proxy, z = median_proxy)) +
  # CTmin for FINITE sigmaH:
  geom_contour_filled(
    data = filter(CTmin_df,
                  sigmaH_proxy < 10^2.2),
    breaks = CTmin_breaks
    ) +
...
minCTmax <- floor(min(filter(CTmax_df, is.finite(median))$median))
...
CTRange_df <- read_rds("results/CT_vals.rds") %>%
  pathosystem_labeller() %>%
  # Arrange along the plotting variables
  ...
  pivot_wider(id_cols = c("system_label", "sigmaH", "KH"),
...
```{r fig-S7_CT_distributions, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:CT_dists} The distributions of the critical thermal minimum, critical thermal maximum, and thermal optimum vary with vertebrate host availability.'}
...
```{r fig-S8_R0_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:R0_sens} The sensitivity of the distribution of $\\mathcal{R}_0$ to mosquito life history traits.'}

# !!! [] change factor/levels for focal_var so that they show up in the same
#        order as in the legend
# !!! [] figure out what's going on with the "color" bars occuring at low temperatures for plots (a) (b) and (e)
...
```{r fig-S9_ddTR0_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:ddTR0_sens} The sensitivity of the distribution of the derivative of $\\mathcal{R}_0$ with respect to temperature to mosquito life history traits.'}

# !!! [] just compare "low host availability (sigmaH = 50, KH = 1 or 10)" and
#        "high host availability (sigmaH = Inf, KH = 100)" scenarios
...
```{r fig-S10_Topt_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:Topt_sens} The sensitivity of the thermal optimum to mosquito life history traits as a function of vertebrate host population density when $\\sigma_H = 50$.'}
...
```{r fig-S10_2_dToptdKH_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:Topt_sens} The sensitivity of the thermal optimum to mosquito life history traits as a function of vertebrate host population density when $\\sigma_H = 50$.'}



dToptdKH.HPD <- read_rds("results/dToptdKH_HPD_sens.rds") %>%
  ungroup() %>%
  distinct() %>% 
  filter(!is.na(HPD_width)) %>% 
  pathosystem_labeller() %>%
...
```{r fig-S11_CTmin_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:CTmin_sens} The sensitivity of the critical thermal minimum to mosquito life history traits as a function of vertebrate host population density when $\\sigma_H = 50$ and $\\sigma_H=\\infty$.'}
...
```{r fig-S12_CTmax_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:CTmax_sens} The sensitivity of the critical thermal maximum to mosquito life history traits as a function of vertebrate host population density when $\\sigma_H = 50$ and $\\sigma_H=\\infty$.'}
...
```{r fig-S13_CTwidth_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:CTwidth_sens} The sensitivity of the thermal niche width to mosquito life history traits as a function of vertebrate host population density when $\\sigma_H = 50$ and $\\sigma_H=\\infty$.'}
...
plot.CTwidth.rel.sens <- CT.HPD %>%
  ...
  group_by(system_ID, sigmaH, KH) %>%
  mutate(sens_total = sum(rel_HPD_width)) %>%
  # Divide each sensitivity value by the total sensitiviWty
  mutate(sens_rel = rel_HPD_width / sens_total) %>%
  # Make a stacked plot, colored by parameter
  ggplot(aes(x = KH, y = sens_rel, fill = focal_var)) +
  geom_area() +
  ...
  facet_grid(
    ...
    labeller = labeller(
      system_label = sens_labels,
      sigmaH = as_labeller(
        appender_sigmaH, default = label_parsed)
    )
  ) +
  theme_minimal() +
  theme(
