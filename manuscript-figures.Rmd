...
data.R0_norm <- data.R0_TPCs %>%
  # filter(KH %in% unique(data.R0$KH)[seq(1, length(unique(data.R0$KH)), length.out = 21)]) %>%
  filter(variable == "R0") %>%
  select(-variable) %>%
...
sens_labels <- as_labeller(c(
      ...
      "(c&#41; *Ae. albopictus* and DENV" = "(c&#41; *Ae. albopictus* and  \nDENV",
...
```{r fig-S12_CTmax_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:CTmin_sens} The sensitivity of the critical thermal maximum to mosquito life history traits as a function of vertebrate host population density when $\\sigma_H = 50$ and $\\sigma_H=\\infty$.'}
