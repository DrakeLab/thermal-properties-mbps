...
```{r fig-S8_R0_uncertainty, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:R0_unc} The uncertainty of the distribution of $\\mathcal{R}_0$ to mosquito life history traits.'}
...
```{r fig-S8_2_dR0dvar_uncertainty, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:dR0dvar_unc} The uncertainty in the local sensitivity of $\\mathcal{R}_0$ with respect to mosquito life history traits.'}
...
dR0dk.df <- data.dR0dvar %>%
  filter(KH %in% KH_vals,
         sigmaH %in% c(100, Inf)) %>%
  # filter(median > 0) %>%
  right_join(select(data.R0, system_label, Temperature, Model, sigmaH, KH, median_R0 = median) %>% 
  ...
  # filter(!(focal_var %in% c("Immature development rate", "Prob. of immature survival", "Eggs per female per day", "Vector competence", "Adult lifespan"))) %>% 
  arrange(sign_median) %>% 
  arrange(Temperature) 
...
plot.dR0dk <- dR0dk.df  %>% 
  # Make a stacked plot, colored by parameter
  ggplot(aes(x = Temperature, y = abs_median, color = focal_var, linetype = (as.factor(sign_median)))) +
  geom_line(lwd = 1) +
  scale_color_manual(
    name = "Focal trait:",
    values = rev(c4a("misc.okabe", 7))
  ...
  scale_y_continuous(
    name = unname(TeX("Local sensitivity of $R_0$")),
    limits = 10^c(-4, 3),
    breaks = 10^(seq(-4,3)), 
    labels = TeX(c("$10^{-4}$", "$10^{-3}$", "$10^{-2}$", "$10^{-1}$", "$10^0$", "$10^1$", "$10^2$", "$10^3$")),
    trans = 'log10',
    expand = c(0,0)
  ) +
  scale_linetype_manual(
...
```{r fig-S10_Topt_uncertainty, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:Topt_unc} The uncertainty of the thermal optimum to mosquito life history traits as a function of vertebrate host population density when $\\sigma_H = 50$.'}
...
```{r fig-S10_2_dToptdKH_uncertainty, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:Topt_unc} The uncertainty in the sensitivity of the thermal optimum to host availability as a function of vertebrate host population density when $\\sigma_H = 50$.'}
...
```{r fig-S12_CTmax_uncertainty, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:CTmax_unc} The uncertainty of the critical thermal maximum to mosquito life history traits as a function of vertebrate host population density when $\\sigma_H = 50$ and $\\sigma_H=\\infty$.'}
