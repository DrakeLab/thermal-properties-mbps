...
data.in.transform <- read_rds(here::here("data/clean/TPC_param_samples.rds"))
...
# Helper function: add a "point at infinity" for plotting when sigmaH is infinite
inf_skew_func <- function(df, skew, divide, n_rows_to_add) {
  ...
  # Create a proxy sigmaH variable that replaces Inf with skew
...
plot_bool = FALSE
source("code/trait-transform.R", local = TRUE)
...
# Get data for plotting thermal performance curves (data.Vec)
source("code/trait-variation.R", local = TRUE)

# Load output data -------------------------------------------------------------
# These data are produced by the code in code/get-outputs.R
...
## Get mean values of vector traits across samples
mean.Vec <- data.Vec %>% 
...
MosqThermRange_df <- mean.Vec %>%
  mutate(system_label = case_when(
    ...
    system_ID == "Anopheles gambiae / Plasmodium falciparum" ~ "(d) *Anopheles* spp. & *P. falciparum*",
...
```{r fig-3_MosquitoThermalTraits, dev = c('pdf','svg', 'png'),  out.width='100%', fig.height=6, fig.width=9, fig.align='center', fig.cap='\\label{fig:MosquitoThermalTraits} Thermal performance curves for the mosquito and parasite traits used in our model. '}
...
MosqThermTraits_plot <- MosqThermTraits_df %>%
  ...
  scale_color_manual(
    ...
    labels = function(x) {
      case_when(
        x == "Aedes aegypti / DENV" ~ "(a) *Ae. aegypti* & DENV",
        x == "Aedes aegypti / ZIKV" ~ "(b) *Ae. aegypti* & ZIKV",
        x == "Aedes albopictus / DENV" ~ "(c) *Ae. albopictus* & DENV",
        x == "Anopheles gambiae / Plasmodium falciparum" ~ "(d) *Anopheles* spp. & *P. falciparum*",
        x == "Culex quinquefasciatus / WNV" ~ "(e) *Cx. quinquefasciatus* & WNV"
      )
    }
  ) +
...
```{r fig-5_Topt, dev = c('pdf','svg', 'png'), dev="cairo_pdf", message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:Topt} The thermal optimum for transmission, $T_{opt}$, may be sensitive to vertebrate host population density and biting tolerance.'}
...
# Value of sigmaH to replace 'Inf' with for plotting
infinite_skew <- 10^log10(max_sigmaH*1.5) # 1.1*max_sigmaH #10^2 + 80
# Used to create a gap dividing finite and infinite sigmaH
infinite_divide <- 10^log10(max_sigmaH*1)

# infinite_skew <- 10^2 + 150
Toptalt_df <- inf_skew_func(Toptalt_df, infinite_skew, infinite_divide, 8)

# infinite_divide <- 10^mean(c(2, log10(infinite_skew)))

# Define sigmaH tick breaks and labels
sigmaH_breaks <- c(10^c(seq(-1, 2, by = 1)), mean(c(infinite_divide, infinite_skew)), max(Toptalt_df$sigmaH_proxy))
...
```{r fig-6_CT_Width, dev = c('pdf','svg', 'png'), dev="cairo_pdf", message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:CT_Width} The parasite population thermal tolerance range, the range of temperatures at which parasite transmission is expected to persist, is widest when biting tolerance is high and vertebrate host population density is low.'}
