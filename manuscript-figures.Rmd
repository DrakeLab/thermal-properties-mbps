...
temp_lims <- data.dR0dvar %>% 
  filter(focal_var == "Adult mortality rate") %>% 
  filter(!is.na(median)) %>% 
...
val_lims <- tibble(sigmaH = c(100, Inf), 
...
plot.dR0dk <- dR0dk.df %>%
  # filter(focal_var %in% c("Adult mortality rate")) %>%
