...
# Used to create a gap dividing finite and infinite sigmaH
infinite_divide <- 10^3.5

R0_df <- inf_skew_func(R0_df, infinite_skew, infinite_divide, 3)
...
R0_plot <- R0_df %>%
  ...
  geom_hline(
    ...
    mapping = aes(yintercept = CTmin_V0, colour = "VTTR"),
    lwd = 1.275
  ) +
  geom_hline(
    data = MosqThermRange_df,
    mapping = aes(yintercept = CTmax_V0, colour = "VTTR"),
    lwd = 1.275
  ) +
  ...
  # Topt curve:
  geom_path(
    data = filter(Topt_for_R0_df, KH %in% c(.1, 1, 10)),
    mapping = aes(
      x = sigmaH_proxy,
      y = median,
      colour = "Topt"
...
plot.densities_violin_inline <- ggplot() +
  ...
  facet_wrap(~system_label, labeller = labeller(), scales = "free") +
  theme_minimal_grid() +
  theme(
    legend.margin = margin(0, 0, 0, 0),
    legend.title = element_text(size = 10),
...
plot.R0.rel.sens <- R0.HPD %>%
  filter(KH %in% c(100) & sigmaH %in% c(50, Inf)) %>%
  ...
  scale_fill_discrete(
    name = "Focal trait:",
    breaks = c("muV", "sigmaV", "sigmaV_f", "rhoL", "deltaL", "etaV", "betaV"),
    labels = c(
      "Adult lifespan", "Biting frequency", "Eggs per female per day",
      "Immature development rate", "Prob. of immature survival",
      "Prob. of surviving EIP", "Vector competence")
  ) +
  scale_x_continuous(
...
```{r fig-S13_CTwidth_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:CTmin_sens} The sensitivity of the thermal niche width to mosquito life history traits as a function of vertebrate host population density when $\\sigma_H = 50$ and $\\sigma_H=\\infty$.'}
