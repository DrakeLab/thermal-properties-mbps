...
get.thermal.response <- function(SpeciesName, PathogenName, ParameterName) {
  df <- filter(
    MosqThermParms, Species == SpeciesName, Pathogen == PathogenName,
  ...
  if (dim(df)[1] == 0) {
    df <- filter(
      MosqThermParms, Species == SpeciesName,
...
out <- tibble(
  Mosquito_species = "", Pathogen = "", Temperature = numeric(),
...
for (ii in 1:dim(MosqPathPairs)[1])
{
  ...
  get.parameter <- function(ParameterName) {
    as.double(get.thermal.response(
      Mosquito_species, Pathogen_name,
      ParameterName
    )(Temperature_vector))
  }
  ...
  if (Mosquito_species == "Anopheles spp.") {
    muV <- -log(get.parameter("p"))
  }
